...
void UIComponent::turn_on_led(grid_section_index_t index) {
  io->grid->turn_on
    ({ .layout  = layout,
       .section = section,
       .index   = index,
...
void UIComponent::turn_on_leds(std::vector<grid_section_index_t> indices) {
  for (auto index : indices) {
    turn_on_led(index);
  }
}

void UIComponent::turn_off_led(grid_section_index_t index) {
  // removes animation and sets intensity to 0
  remove_animation(index, 0);
}

void UIComponent::set_led(grid_section_index_t index, unsigned int intensity) {
  io->grid->set
    ({ .layout  = layout,
       .section = section,
       .index   = index,
...
void UIComponent::set_leds(std::vector<grid_section_index_t> indices, unsigned int intensity) {
  for (auto index : indices) {
    set_led(index, intensity);
  }
}

void UIComponent::add_animation(std::shared_ptr<Animation> animation, grid_section_index_t index) {
...
void UIComponent::add_animation(std::map<grid_section_index_t, std::shared_ptr<Animation> > index_to_animation) {
...
void UIComponent::remove_animation(grid_section_index_t index) {
...
void UIComponent::remove_animation(grid_section_index_t index, unsigned int intensity) {
  io->grid->animation->remove
    ({ .layout  = layout,
       .section = section,
       .index   = index,
...
void UIComponent::remove_animation(std::vector<grid_section_index_t> indices) {
...
void UIComponent::remove_animation(std::vector<grid_section_index_t> indices, unsigned int intensity) {
  std::vector<grid_addr_t> grid_addrs;
...
bool UIComponent::get_section_size(std::vector< std::shared_ptr<Layout> > layouts) {
  ...
  for (auto candidate_layout : layouts) {
    if (candidate_layout->name == layout) {
      ...
      if (get_section_size(candidate_layout->sublayouts)) return true;

      // otherwise, keep looping and searching.
    }
  }
