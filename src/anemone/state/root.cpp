...
State::Root::Root() {
  ...
  // intialize high frequency state pointers
  step_cursors = std::make_shared<StepCursors>();
  sequences = std::make_shared<Sequences>();
  
  // get high frequency state ptrs
  std::shared_ptr<State::step_cursors_t> step_cursors_ptr = step_cursors->state->get();
  std::shared_ptr<State::sequences_t> sequences_ptr = sequences->state->get();

  // instantiate root composite state
  state = rx::State<root_t>
    ::with_actions<action_t>
    ::compose<sequencer_t, instruments_t>
    ([step_cursors_ptr, sequences_ptr] (sequencer_t s, instruments_t i) -> root_t {
       return {
               .sequencer    = s,
               .instruments  = i,
