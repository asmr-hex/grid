...
SCENARIO( "Sequencer driven by multiple clocks" ) {
  GIVEN( "two clocks and a Sequencer" ) {
    ...
    s->sync("A", clkA.get_observable().skip(1));
    s->sync("B", clkB.get_observable().skip(1));
