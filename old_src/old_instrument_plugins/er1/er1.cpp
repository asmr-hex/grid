    ...
    Instrument(create_instrument(InstrumentName::ER1, config, std::make_shared<ER1Layout>(ER1Layout(config))))
{
  ...
  parameters[1]->convert_value_to_midi_events =
    [] (unsigned int value) -> std::vector<midi_event_t> {
      ...
      return {
               { .source = "", .destination = "", .data = nrpn_data_lsb },
               { .source = "", .destination = "", .data = nrpn_data_msb },
               { .source = "", .destination = "", .data = nrpn_data_entry },
      };
  };
